...
- Invariant je **nasazen v kódu (PR #63)**: každé potvrzení ceny nese serverovou auditní
  stopu (kdo/kdy), bulk operace potvrzují jen položky s explicitní per-item attestací,
  slepé „Potvrdit vše" neexistuje, změna produktu nebo ceny potvrzení automaticky ruší.
...
attestací a auditní stopou (PR #63 — invariant §0 nasazen)**, monitoring feed z NEN
...
| 8 | Škálování (vstup vlny E): splněno obchodní kritérium? + zdroje feedu, upsize VPS, denní AI budget | 02 fáze 4, 03 vlna E | před vlnou E |
...
4. **Kódové gaty pilotu**: hard completeness gate s auditovanou výjimkou (A-01),
   feature snapshoty (A-03), základní kill-switch (B-00). Pozn.: 100% per-item review
   s invalidací po změně už NASAZENO (PR #63); hash balíku už v manifestu cockpitu.
...
- **Implementace (03):** nový **A-00 „Připravenost entity a dodávky"** jako tvrdý
  ...
  podáním; **základní kill-switch předsunut do B-00** (před auto-run-all a bulk cesty;
  F-01 na něm staví); GDPR/licence dat/ToS scrapingu (T-06 + A-09 okruh c) **před
  první externí pilot**; SaaS foundation odložena za placenou validaci (první externí
  zákazník = managed service); nové tasky T-08 (SAM) a T-09 (legacy `?token=`).
